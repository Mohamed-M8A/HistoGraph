...
// states claim over time, a small scripting language used to drive those claims tick
// by tick, and a canvas renderer that paints the current ownership state with animated
// color transitions. It is organized into the following sections, in order:
//
...
//      because the UI's Import/Export panel (see domRefs.js's "Importing States" info
//      text and mapEditorPanel.js's MapEditor.importStrict/exportStatesText) is about
//      loading or saving a *palette* — the reusable set of state codes/colors a
//      mapmaker wants to paint with — independently of any specific tick script or
//      map. Mixing this into parsePoliticalScript's tick grammar would force every
//      palette import/export to carry a fake "T0" wrapper it has no use for, and
//      would make exported palettes non-portable across scripts that use different
//      tick numbering. The two grammars intentionally share the same underlying color
//      model (buildStateColors) so a state defined here and one defined via a
//      {STATE ...} block in a tick script always produce byte-for-byte identical
//      base/occupied/advance/border sets.
//
...
//      parseUnitsScript's convention in unitsEngine.js. A state code redefined later
//      in the text simply overwrites the earlier definition in the returned
//      stateColors (last one wins) — this is a straight import/replace operation, not
//      a script the interpreter steps through, so there's no notion of "when" a
//      redefinition takes effect to warn about.
//
...
//      stateColors map that the painter reads from — so a mid-script STATE
...
//        T2 {PEACE FR GER};
//
//   10. Political Painter
//      PoliticalPainter renders the political grid onto a canvas layer: border cells
//      get the border color, cells conquered this tick get the "advance" color, older
//      cells get the "occupied" color, and permanently stabilized cells (via PEACE)
//      get the flat base color. Ownership changes fade in over DEFAULT_TRANSITION_MS
//      using cell.fromColor -> the current target color. Because stateColorMap is
//      read fresh on every rebuild, a STATE redefinition applied by the engine shows
//      up immediately on the cells it affects, without its own fade.
//
//      Performance note: the canvas and its 2D context are created once in the
//      constructor and reused for every rebuild (cleared with clearRect instead of
//      being recreated), to avoid per-frame canvas allocation and the GC pressure /
//      micro-stutter it causes during animation and map zoom/pan.
//
// =====================================================================================
...
function ensureHgpmExtension(filename) {
    return filename.toLowerCase().endsWith(HGPM_EXTENSION) ? filename : `${filename}${HGPM_EXTENSION}`;
}

export function downloadHgpmFile(filename, scriptText) {
    const content = buildHgpmFile(scriptText);
    const blob = new Blob([content], { type: "text/plain" });
    const url = URL.createObjectURL(blob);

    const a = document.createElement("a");
    a.href = url;
    a.download = ensureHgpmExtension(filename);
...
export class PoliticalPainter {
    constructor(canvasWidth, canvasHeight) {
        ...
        // note in section 10 above for why this matters for GC pressure).
