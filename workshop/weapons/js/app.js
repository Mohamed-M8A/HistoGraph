...
function computeBurstTiming(profile) {
    const shotCount = profile.shotCount ?? 6;
    const realRpm = profile.meta && profile.meta.realRpm;
    if (realRpm && realRpm > 0) {
        const interval = 60 / realRpm;
        return { shotCount, interval, totalSec: shotCount * interval, real: true };
    }
    const totalSec = (profile.durationMs / 1000) * 0.8;
    return { shotCount, interval: totalSec / shotCount, totalSec, real: false };
}

function synthBurst(ctx, dest, profile) {
    const preset = getPreset(profile);
    const timing = computeBurstTiming(profile);
    const totalSec = timing.totalSec;
    const shotCount = timing.shotCount;
    ...
    for (let i = 0; i < shotCount; i++) {
        ...
        const fatigue = 1 - (i / Math.max(1, shotCount - 1)) * 0.06;
        const freq = jitter(baseCrackFreq, 0.07);
        const shotQ = jitter(0.8, 0.28);
        const shotAttack = jitter(0.0022, 0.35);
        crackClick(ctx, dest, { start, peak: 0.4 * fatigue, roughness: roughness * 0.5 });
        noiseBurst(ctx, dest, {
            start, duration: crackDur,
            freq, freqEnd: 700, Q: shotQ,
            attack: shotAttack,
            decay: crackDur * 0.7, peak: jitter(0.85, 0.06) * fatigue,
            roughness
        });
        if (preset?.mechanical) {
            noiseBurst(ctx, dest, {
                start: start + crackDur * jitter(0.6, 0.2), duration: 0.02,
                filterType: "bandpass", freq: jitter(650, 0.15), Q: 0.9,
                attack: 0.001, decay: 0.018, peak: 0.05 * fatigue,
                roughness: Math.min(roughness * 0.4, 0.1)
            });
        }
        if (bodyFreq) {
            if (preset?.mechanical) {
                subBoom(ctx, dest, { start, duration: crackDur * 1.3, freqStart: jitter(bodyFreq, 0.1), freqEnd: bodyFreq * 0.4, peak: boomPeak * 0.93 * fatigue, roughness: roughness * boomRoughnessMult });
                subBoom(ctx, dest, { start: start + crackDur * 0.35, duration: crackDur, freqStart: jitter(bodyFreq * 0.7, 0.1), freqEnd: bodyFreq * 0.3, peak: boomPeak * 0.53 * fatigue });
            } else {
                subBoom(ctx, dest, { start, duration: crackDur * 1.3, freqStart: jitter(bodyFreq, 0.1), freqEnd: bodyFreq * 0.4, peak: boomPeak * fatigue, roughness: roughness * boomRoughnessMult });
            }
        }
    }
...
function estimateTailSeconds(profile) {
    let base = (profile.durationMs ?? 800) / 1000;
    if (profile.kind === "burst") {
        const timing = computeBurstTiming(profile);
        base = Math.max(base, timing.totalSec);
    }
